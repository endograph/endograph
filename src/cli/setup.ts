...
export function fromTemplate(dir: string, template: string): string | null {
  ...
  if (!existsSync(join(src, "endograph.toml"))) return `${src} has no endograph.toml`;
  const files = ["endograph.toml", ...(existsSync(join(src, "manifest.md")) ? ["manifest.md"] : [])];
  for (const f of files) if (existsSync(join(dir, f))) return `${join(dir, f)} already exists; refusing to overwrite`;
